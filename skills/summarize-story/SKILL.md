...
description: Convert storytelling podcasts, company histories, entrepreneurial journeys, and business narratives into structured story-focused notes
license: MIT
metadata:
...
# Narrative Content Summarizer

Specialized skill for converting storytelling podcast episodes, company histories, entrepreneurial journeys, and business narratives into structured, story-focused reference notes.

## When to Load This Skill

Load this skill when the user wants to summarize:

- Podcast episodes (Acquired, How I Built This, Masters of Scale, etc.)
- Company histories and founding stories
- Entrepreneurial journeys and founder stories
- Business strategy discussions
- Industry evolution stories
- Any narrative content with business lessons
...
- "business journey"
- "founding story"
- User provides podcast episode transcript and asks for notes
...
1. **Gather content**: Get the podcast transcript or story content
2. **If URL provided**: Use the `braindead fetch` command or helper scripts to fetch content
   - YouTube: `braindead fetch <url> --video --json`
3. **Prepare metadata**: Extract relevant information (podcast name, episode title, date, duration, URL)
4. **Apply prompt template**: Use the prompt template provided below with the content and metadata
5. **Format output**: Return as story-focused structured markdown notes

## Prompt Template

Use this prompt template when processing narrative content:
...
[Additional thematic sections following the same structure, maintaining chronological flow within sections but organizing by major themes, eras, or business phases]
...
[Any other relevant context, updates since the episode aired, interesting asides, or meta-commentary about the episode that doesn't fit above categories]
`````
...
- Story-focused structure organized by themes/eras
- Preserved financial metrics and key numbers
- Well-organized sections including lessons, people, companies
- In the same language as the input content
- Saveable directly as a .md file

## Integration Notes

- Works with webfetch to research source context
- Can chain with the `braindead fetch` command for URL input
- Inherits conversation context about the story
- Focuses on extracting business lessons and strategic insights
```
